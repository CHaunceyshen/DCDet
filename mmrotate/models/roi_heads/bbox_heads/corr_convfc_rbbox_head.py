...
class RotatedCorrConvFCBBoxHead(RotatedCorrBBoxHead):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(RotatedCorrConvFCBBoxHead, self).__init__(
...
@ROTATED_HEADS.register_module()
class RotatedCorrShared2FCBBoxHead(RotatedCorrConvFCBBoxHead):
    """Shared2FC RBBox head."""

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(RotatedCorrShared2FCBBoxHead, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
...
@ROTATED_HEADS.register_module()
class RotatedCorrKFIoUShared2FCBBoxHead(RotatedCorrShared2FCBBoxHead):
    """KFIoU RoI head."""

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(RotatedCorrKFIoUShared2FCBBoxHead, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
    ...
    def loss(self,
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                ious = rbbox_overlaps(
                    pos_bbox_pred, pos_bbox_targets, is_aligned=True).clamp(min=1e-5).detach()
                labels_pos = labels[pos_inds]
                cls_score_lab = cls_score[pos_inds, labels_pos]
                cls_score_ult = cls_score_lab.sigmoid()
                loss_crloss = spearmanr(cls_score_ult, ious)
                losses['loss_crloss'] = loss_crloss * 0.3
            else:
